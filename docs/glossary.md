...
Reveal flow 中的一个 **触发步骤**。一张 slide 可以有 0 到多个 cue。Cue 的总数由 frontmatter `clicks` 和实际检测到的 `<Reveal>` 数量取较大值决定。
...
1. **核心组件**（始终可用）：`Badge`、`Callout`、`Reveal`、`RevealGroup`、`Annotate` 等
...
点击触发的内容显隐机制。使用 `<Reveal step={n}>` 标签包裹需要按步骤显示的内容。`step` 对应 cueIndex。

### RevealGroup

自动编号的 Reveal 容器。内部的 `<Reveal>` 子元素会被自动分配递增的 step 值。
